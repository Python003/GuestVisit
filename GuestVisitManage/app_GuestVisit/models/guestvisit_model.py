...
def appointsearch():
	...
	try:
		...
		sql = 'select * from( \
				...
				from t_visit_apply tva,m_user u where tva.schedule_date = \'{0}\' \
				and tva.employee_id = u.user_id) as a \
				left join \
...
def record_search(visitor_name,begindatetime,enddatetime,visit_status):
	...
	sql +=	'order by tva.schedule_date desc, tva.schedule_time desc) as tvr_in \
			...
			on tvr_in.data_seq = tvr_out.data_seq and tvr_in.update_at < tvr_out.update_at_out \
			order by tvr_in.update_at desc'.format(begindatetime,enddatetime)
...
def get_appoint_data(user_id,begindate,enddate):
	...
	sql = 'select data_seq, schedule_date, schedule_time, company, visitor, identity_no, approve_status from \
		...
		'.format(user_id,begindate,enddate)
	try:
		cur = connections['GuestVisitManage'].cursor()
		cur.execute(sql)
		rows = cur.fetchall()
